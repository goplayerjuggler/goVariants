(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.go_variants_editor = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
...
function transformer(options) {
  ...
  */
    ...
    */
				...
				if (node.SC) {
					var score = engine.score(stonesMarkedForScoring);
					if (node.SC & 1 === 1) {
						updatedComment = true;
						comment(isPass, isBlack, score.RE);
					} else /*don't want to treat succint and verbose at the same time*/if (node.SC & 2 === 2) {
							updatedComment = true;
							comment(isPass, isBlack, 'Black: ' + (score.totalWhiteDead + score.totalWhiteCaptured + score.totalBlackTerritory) + ' = ' + score.totalBlackTerritory + ' territory + ' + (score.totalWhiteDead + score.totalWhiteCaptured) + ' prisoners\nWhite: ' + (score.totalBlackDead + score.totalBlackCaptured + score.totalWhiteTerritory) + ' = ' + score.totalWhiteTerritory + ' territory + ' + (score.totalBlackDead + score.totalBlackCaptured) + ' prisoners +' + engine.rules.komi + ' komi');
						}
					if (node.SC & 4 === 4) {
						wrappedGame.game.nodes[0].RE = score.RE;
					}
				}
				if (!updatedComment) comment(isPass, isBlack);
...
},{"go-variants-engine":153,"lodash/findIndex":259,"lodash/flatten":260,"lodash/uniqBy":284,"smartgame":285,"smartgamer":286}],288:[function(require,module,exports){
'use strict';
...
document.goVariantsEditor = function (editorOptions) {
	editorOptions = _extends({ rootId: 'sgfEditor', showExtras: true }, editorOptions);

	var _editorOptions = editorOptions,
	    rootId = _editorOptions.rootId,
	    showExtras = _editorOptions.showExtras,
	...
	var getElementByIdSuffix = function getElementByIdSuffix(suffix) {
		return document.getElementById(rootId + '_' + suffix);
	};

	getElementByIdSuffix('updateButton').addEventListener('click', updateVariantSgf);[].forEach.call(document.querySelectorAll('#' + rootId + '_viewerControls input[type=button]'), function (el) {
	...
	if (showExtras) {
		...
		var select = getElementByIdSuffix('sizeSelect');
	...
	function showBoard(options) {
		...
		var _options = options,
		    tSgf = _options.tSgf,
		    panningDirection = _options.panningDirection,
		    moveReference = _options.moveReference,
		    reset = _options.reset;

		if (reset || !viewer.offset) viewer.offset = [0, 0];
		if (panningDirection) {
			var right = viewer.offset[0],
			    up = viewer.offset[1];
		...
		try {
			var transformer = go_variants_transformer({
				...
				wraparoundMarkersType: wraparoundMarkersType,
		...
		if (!panningDirection) {
			...
			GoBoardApi.Set_OnGameTreeModifiedCallback(oGameTree, function () {});
			...
			if (moveReference !== undefined) GoBoardApi.Load_Sgf(viewer.oGameTree, sgf, undefined, moveReference);else GoBoardApi.Load_Sgf(oGameTree, sgf);
			...
			GoBoardApi.Set_OnGameTreeModifiedCallback(viewer.oGameTree, function () {});
			...
			getElementByIdSuffix('offsetTr').innerHTML = 'panned: [' + viewer.transformer.modX(viewer.transformer.options.projectionSettings.offset[0]) + ', ' + viewer.transformer.modY(-viewer.transformer.options.projectionSettings.offset[1]) + ']';
		}
	}
	...
	function updateVariantSgf() {
		...
		getElementByIdSuffix("sgfIn").value = tSgf;
		showBoard({ tSgf: tSgf, moveReference: moveReference });
	}
	...
	function getLittleGolemSgfAndShowBoard() {
		...
		var failMsg = 'load from littel Golem failed';
		fetch(proxyurl + url, { headers: myHeaders }).then(function (response) {
...
},{"../src/transformer":287,"./editor.jsx":289}],289:[function(require,module,exports){
"use strict";

module.exports = function (rootId) {
    return React.createElement(
        ...
        React.createElement(
            ...
            React.createElement(
                ...
                React.createElement("textarea", { id: rootId + "_sgfIn", rows: "3" }),
                React.createElement("input", { type: "button", id: rootId + "_goButton", value: "show board (from t-Go SGF)" })
            ),
            React.createElement(
                "div",
                { "class": "item go-variants-extras" },
                React.createElement(
                    "label",
                    { "for": rootId + "_littleGolemId" },
                    "LittleGolem game ID:"
                ),
                React.createElement("input", { id: rootId + "_littleGolemId", type: "text" }),
                React.createElement("input", { type: "button", id: rootId + "_goLgButton", value: "show board (from littleGolem game ID)" }),
                React.createElement("span", { id: rootId + "_goLgMsg" })
            ),
            React.createElement(
                "div",
                { "class": "item go-variants-extras" },
                React.createElement(
                    "label",
                    { "for": rootId + "_sizeSelect" },
                    "Size:"
                ),
                React.createElement("select", { id: rootId + "_sizeSelect" }),
                React.createElement("input", { type: "button", id: rootId + "_newButton", value: "New game" })
            ),
            React.createElement(
                "div",
                { style: "display: none", id: rootId + "_transformedSgfFs", "class": "item go-variants-extras" },
                React.createElement(
                    "label",
                    { "for": rootId + "_sgfOut" },
                    "Transformed SGF:"
                ),
                React.createElement("textarea", { id: rootId + "_sgfOut", rows: "3" })
            ),
            React.createElement(
                "div",
                { id: rootId + "_viewerControls", style: "display:none", "class": "item" },
                React.createElement(
                    "table",
                    { title: "panning", style: "float:left" },
                    React.createElement(
                        "tr",
                        null,
                        React.createElement(
                        ...
                        React.createElement(
                            "td",
                            { rowspan: "4", style: "vertical-align: middle" },
                            "\u201CPanning\u201D",
                            React.createElement(
                                "p",
                                null,
                                " Use the buttons to the left to \u201Cpan\u201D the board."
                            )
                        )
                    ),
                    React.createElement(
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            "td",
                            { colspan: "2", style: "text-align: center; font-size: small", id: rootId + "_offsetTr" },
                            "panned: [0,0]"
                        )
                    )
                )
            ...
            React.createElement(
                ...
                React.createElement(
                    "label",
                    { "for": rootId + "_wraparoundSelect" },
                    "number of wraparound lines to add:"
                ),
                React.createElement(
                    "select",
                    { id: rootId + "_wraparoundSelect" },
                    React.createElement(
                        "option",
                        { value: "0" },
                        "0"
                ...
                React.createElement("input", { type: "checkbox", id: rootId + "_addComments" }),
                React.createElement("br", null),
                React.createElement(
                    "label",
                    { "for": rootId + "_wraparoundBorderSelect" },
                    "Type of border for wraparound:"
                ),
                React.createElement(
                    "select",
                    { id: rootId + "_wraparoundBorderSelect" },
                    React.createElement(
                        "option",
                        { value: "0" },
                        "No border"
                    ...
                    React.createElement(
                        "option",
                        { value: "2" },
                        "Partial border (corners & middles)"
                    ),
                    React.createElement(
                        "option",
                        { value: "3" },
                ...
                React.createElement(
                    "label",
                    { "for": rootId + "_coordinateSelect" },
                    "Type of coordinates:"
                ),
                React.createElement(
                    "select",
                    { id: rootId + "_coordinateSelect" },
                    React.createElement(
                        "option",
                    ...
                    React.createElement(
                        "option",
                        { value: "2" },
                        "(\u2192\u2191;A|1-L|11): Western, no \u201CI\u201D"
                    ),
                    React.createElement(
                        "option",
                        { value: "3" },
                        "(\u2192\u2193;1|1-11|11): Latin/Latin, top to bottom"
                    ),
                    React.createElement(
                        "option",
                        { value: "4" },
                        "(\u2192\u2193;1|1-11|\u5341\u4E00): Latin/Chinese, top to bottom"
                    )
                ),
                React.createElement("br", null),
                React.createElement(
                    "div",
                    { id: rootId + "_updateButtonDiv", style: "display:none" },
                    React.createElement("input", { type: "button", id: rootId + "_updateButton", value: "update (from board)" }),
                    " Press this button after adding marks to the board position in order to get the wraparound effect."
                )
            )
        ),
        React.createElement("div", { id: rootId + "_playerDiv", style: "position:relative; height: 75vh;width: 85vw" })
    );
};
